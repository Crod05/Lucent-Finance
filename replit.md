...
- **Auth foundation (Session A, no auth yet)**: `users` table (uuid PK, `UNIQUE(auth_provider, auth_provider_subject)`, status CHECK) plus nullable `user_id uuid` FKs (ON DELETE RESTRICT) on accounts/transactions/budgets/bills — migration `0003_dapper_patch` deterministically backfills the FOUR FINANCIAL tables only to the fixed legacy owner `00000000-0000-4000-8000-000000000001` (status `migration_pending`). Gamification tables are NOT migrated in Session A (rows stay on `'default-user'`, columns stay text with their defaults, existing XP/achievements/missions remain fully visible) — their ownership migration is a Session B hard gate, atomic with `DEFAULT_USER` removal + userId propagation + text→uuid conversion. Runtime is UNCHANGED: writers do not set `user_id`, fingerprint uniqueness is still global (gate documented in `transactions.ts`). Verify with `pnpm --filter @workspace/scripts run verify-session-a` (exit 0 with reported `[ANOMALY]` lines for known pre-existing issues, e.g. the orphaned `bonus_missions` id 2 → `transaction:29` ref; exit 1 only on real failures). Test users seeded in vitest global-setup (`__tests__/fixtures/users.ts`). Full plan + amended implementation report: `docs/authentication-and-user-scoping.md`.
